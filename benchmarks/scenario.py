...
ITEMS: dict[str, dict] = {
    ...
    "law-cheap": {"kind": "law", "topic": "principles", "confidence": 0.9,
                  "text": "Check the cheapest explanation before the expensive one",
                  "needle": "cheapest explanation"},
    "law-undo": {"kind": "law", "topic": "principles", "confidence": 0.85,
                 "text": "When uncertain, prefer the action that is easiest to undo",
                 "needle": "easiest to undo"},
...
_SAY_ON_DAY = {
    0: ["me", "boss", "daughter", "editor", "tz", "company", "project", "deploy", "weekly",
        "changelog", "law-cheap", "law-undo"],
    1: ["coffee-sam"],
    2: ["creds"],
    5: ["ratelimit"],
    6: ["oncall"],
...
_PROBE_ON_DAY = {
    ...
    20: ("the new vendor integration is failing in a way nobody has seen before, where do I even start?",
         ["law-cheap"]),
    ...
    43: ("weird spike overnight and no runbook covers it — hot-patch prod directly or hold off?",
         ["law-undo"]),
